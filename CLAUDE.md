...
| `/ctp <sub>` | **Cross-shard capability transport** (#173, [CapabilityTransport.md](CapabilityTransport.md)) — a bridge is a dual-shard account, its room derived from `owner id + shard pair`. `new <shardA-url> <shardB-url> [--group <id>]` · `list` · `show` · `join <pair>` · `setup <pair>` (the escrow deploy recipe) · `escrow <pair> <A\|B> <uri>` · `send <amount> to <A\|B> [<destAddr>]` · `lock <id>` · `mint <id>` · `status` · `receipts` · `abort <id>`. `send`/`setup --deploy` drive both shards' legs themselves via `deployToNode` (`rholang.ts`, per-node result nonce); `--manual` prints the `/rholang` recipe instead. Room coordination + audit via `ctp-*` broadcast. rholang escrow is `ctp-escrow.js`, verified end to end against a live rnode with real funded deploys by `scripts/localnet/ctp-e2e.mjs` |
