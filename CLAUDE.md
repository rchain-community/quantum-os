...
- **Issuer-backed tokens** (`wrapped-token.js`, imported by `rholang-macros.js`) — two uses. `$winstall(backingAddr, baseCurrency)` deploys a fresh mint/burn/balanceOf contract, issuer-gated to the deployer. **A personal currency**: `$mint(wrapperUri, amount, holder)` credits any address directly, no `revVault` involved — issued on the issuer's own word, the on-chain analog of a `/note` currency (`ExchangeDemo.md` §1). **A wrapped native token**: REV (or any chain's own) trades on the exchange only wrapped — `$wrap` is the **one** macro in this whole design that touches `revVault`, because it's the issuer's own sanctioned deploy: a real transfer to the wrapper's backing address, chained with `mint` in one program. `$unwrap` is the holder's own `burn`, self-identified on-chain via `rho:rev:address "fromDeployerId"` (never a caller-supplied string). `$wrelease` is the issuer's separate redemption step (their own transfer, chained with marking the claim released) — two macros because they're two parties' actions. `$winfo`/`$wbalance` are reads; `info` reports the backing address so anyone can `$balance` it against supply — public verifiability, not enforcement. Fungibility: `w<BASE>~<issuer8>`, the terms-series shape. Verified end to end against localnet (wrap → mint → burn → release).
