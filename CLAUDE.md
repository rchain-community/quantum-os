...
| `$name(args)` | Run a **chain macro** — `$verify(@x)` (local), `$balance(me)` / `$xquote(…)` (unsigned `/rholang eval`), `$transfer(10, "1111…")` / `$xswap(…)` / `$anchor(…)` (signed deploy), `$( … )` (inline program). Built-ins in `rholang-macros.js` (+ `$x*` exchange in `rholang-exchange.js`); unknown names fall through to this room's `/macro` fragments. `$` is illegal in rholang so a missed expansion fails loud |
...
| `packages/browser/src/rholang-macros.js` | **The `%` capability macro registry — one source for both halves.** Plain JS, ZFA kernel injected via `createMacroEngine(kernel)`; one sibling import (`rholang-exchange.js`, also plain JS) for the `$x*` shape. Registry + arg validators + templates + the rholang call-site scanner (`expandProgram`). Edit macros here and nowhere else; `node scripts/qos-cli/rholang-macros.mjs --selftest` covers it |
| `packages/browser/src/rholang-exchange.js` | **The pooled token exchange contract + `$x*` call-site builders.** Plain JS, no imports, `--selftest` (in CI). `EXCHANGE_RHO` (open/provide/deposit/quote/swap/withdraw/link/route/setRate/inspect — method dispatch, consumed-and-restored state cells, ≥2 params, no quoted names, never `revVault`) + `installProgram` / `*Program` builders. Federates across shards via a remote signed deploy (rchain-rust#33). Verified end to end against localnet |
...
| `ExchangeDemo.md` | End-user walkthrough of the pooled token exchange (`rholang-exchange.js` / `$x*` macros) — open/provide/deposit/quote/swap/withdraw, cross-shard `link`/`route` federation over Layer 1, the `/note`-currency connection, the no-REV constraint. Update when a verb, a `$x*` macro, or the federation model changes |
