...
- **`$name(…)` as a room line** (`runDollarLine` in `app.ts`, routed from `runInput` alongside `+`): `$verify(@x)` answers locally; `$balance(me)` runs an unsigned `/rholang eval` (`me` → this browser's REV address, since `rho:rchain:deployerId` is unbound in an exploratory deploy); `$transfer(10, "1111…")` and `$anchor(…)` open the deploy path; `$( new x in { $anchor(…) } )` is a full inline program (deploy unless every site is a read). A `$name` the built-in library doesn't know falls through to this room's `/macro`-defined ones.
