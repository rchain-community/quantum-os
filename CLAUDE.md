...
- **Commands** (user-invoked, relayed by the browser as chat — `handleCommand` relays `facil`/`facilitator`/`scribe`/`skeptic`/`greeter`): `/<cmd>` (present?), `/<cmd> help`, `/<cmd> ask <q>`, **`/<cmd> optimize <problem>`**, **`/<cmd> chair <topic>`** (+ `next`/`back`/`close`/`cancel` to steer it), `/<cmd> trust`, **`/<cmd> health`** (uptime/RSS/CPU, signaling + channel state, present peers, budget used, trust standing, last error — CPU as a lifetime average *and* a delta since the previous check, since the average hides a runaway the way `ps` %CPU hid the orphaned-SCTP burn), **`/<cmd> list [n]`** (the room's screen history the agent has seen — chat lines and `/command → result` lines, oldest→newest; default 25, max 500; a 1:1 answer to the asker, not a room broadcast; the agent's own control replies and `/<cmd> …` commands are excluded, kept in a count-bounded `transcript` buffer, not time-purged like `recentMsgs`), `/<cmd> off|on`, and — only when the agent was started with `--key` (TEST SYSTEMS ONLY) — **`/<cmd> faucet [address]`**: signs and deploys a fixed-amount REV transfer. **The address is optional and normally omitted** — an agent cannot derive a browser's REV address (the key is the browser's), so `app.ts` fills it in when relaying a bare `faucet`, and `$me` is resolved in any relayed agent command. Being asked "what's your REV address?" by something that could not know it was the bug (`rholang-client.mjs`, reusing the same `$transfer` macro the browser's `/rholang` does), no rate limit; `/<cmd> ask give me some test rev` routes to the identical function via a deterministic regex, never an LLM decision to move funds. Replies bypass the nudge throttle; per-peer self-introductions mention the command + `MyRoom`.
