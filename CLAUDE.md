...
| `/gov <sub>` | **Liquid-democracy + liquid-trust governance** (rgov on primitives) — `new <name>`, `show`, `member add\|remove`, `issue <title>`, `delegate <peer> [on <issue>]`/`undelegate`, **`trust <member> <0–5>`** (confer a level below your own), **`censure\|uncensure <member>`** (⅔-quorum accountability), `vote <issue> \| opts [ranked]`, `treasury declare\|grant\|balance`, `kudos <m> <n>\|balance`, `say <msg>`, `status`, `list`, and **`chain`** — the group's governance ON CHAIN (`chain install` deploys Inbox/Group/Issue, `chain push` writes **your own** rows, `chain pull` reports what the contracts hold, `chain inbox|group|issue <uri>` records one). Push is per-member by construction, not by policy: `delegate`/`rate`/`censure` are self verbs, so an admin cannot push a member's rows and a member needs no admin to push their own. Transitive delegation + trust-weighted tally; treasury/kudos via `/note`; per-group inbox; daemon-persisted; see `Governance.md` and [RGov_Core.md](RGov_Core.md) |
