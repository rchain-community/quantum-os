...
- **`$macro(…) as <pattern> { …block… }`** — capture (`expandProgram` in `rholang-macros.js`). A `capture: true` macro (`$balance` / `$transfer` / `$grant`) binds its result by `<pattern>` and runs the block instead of `return!`ing: `$balance($me) as bal { … }` → `… *ret) | for (@bal <- ret) { <block> }`. `<pattern>` is a name **or** a balanced `(a, b)` / `[a, …rest]` — whatever `for (@<pattern> <- …)` accepts — the `for` fires only when the reply matches the shape, so match what the macro replies (its `help` states it): `$balance` → the int; `$grant` → the raw capability; `$transfer` → a normalised **`(result, error)`** tuple (`result` = `("transfer ok", amt, to)` / `Nil`; `error` = string / `Nil`) for both capture *and* its default reporting. `as { <pat> => …  <pat> => … }` (a `=>` required) drops the binding and runs rholang `match` arms straight against the reply — `… for (@__reply <- <chan>) { match __reply { <arms> } }`. `expand(args, capture)` — `capture` is `{ pattern, block }`, `{ arms }`, or null; the macro turns it into a `for` via `captureSink(capture, chan)` over whichever channel carries the value (`$transfer` normalises the raw `Nil`/string reply through `__outcome` first). Non-`capture` macros reject `as`.
