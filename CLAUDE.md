...
| `/macro <sub>` | **Interact2 — write a `+command`.** `define name(a) body` or `name body` — bare name/params, inline or multi-line body, optional `// doc` (also opens/accepts the `/rholang` editor) · `list` · `show <n>` · `find [re]` · `echo <n> [args]` (expand, run nothing) · `remove <n>`. A body of slash/`+` commands makes a `+command`; a body of rholang makes a `$name(…)` fragment for `/rholang`. Definitions are dyncap-signed room state (`macro-define` / `sync-macros`, tombstoned by `retract`), first-writer-wins by anchor. See [MacRhoLang.md](MacRhoLang.md) / [RChain_Macros.md](RChain_Macros.md) |
