...
- **`$macro(…) as name { …block… }`** — capture (`expandProgram` in `rholang-macros.js`). A `capture: true` macro (`$balance` / `$transfer` / `$grant`) binds its result to `name` and runs the block instead of `return!`ing: `$balance($me) as bal { … }` → `… *ret) | for (@bal <- ret) { <block> }`. `as` gives the **raw** channel reply (`$transfer` → `Nil`/error string), so it's the composition primitive. Without `as`, the macro's own reporting runs. Non-`capture` macros reject `as`. `expand(args, sink)` — the macro splices `${sink}` where its `for` was, defaulting to its own reporting.
