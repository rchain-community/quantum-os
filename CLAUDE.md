...
| `/ctp <sub>` | **Cross-shard value transport** (#173, [CapabilityTransport.md](CapabilityTransport.md)) — a bridge is one account (this browser's `/rholang` key) with a `ctpEscrow` on two rnodes. `new <rnode-A-url> <rnode-B-url>` (reads each node's shard id) · `list` · `show [<pair>]` · `setup [<pair>]` (deploy + cross-register both escrows) · `escrow <pair> <A\|B> <rho:id:…>` · `send [<pair>] <amount> to <A\|B> [<destAddr>] [--manual]` · `lock <id> [<tuple>]` · `mint <id>` · `status` · `receipts` · `abort <id>`. `send` deploys the burn on one node and the mint on the other via `deployToNode` (`rholang.ts`, per-node result nonce) and broadcasts `ctp-receipt` to the current room; `--manual` prints the `/rholang` recipe. Escrow `ctp-escrow.js`, verified end to end across two real nodes with funded deploys by `scripts/localnet/ctp-e2e.mjs`. `deriveBridgeRoom` + `ctp-offer`/`-lock`/`-mint` are in the code, unused — Phase 4 (group-owned bridges) |
...
| `packages/browser/src/ctp.ts` | Cross-shard transport (#173) — bridge identity + `deriveBridgeRoom` (Phase 1) and the transport protocol (Phase 3a): `CtpOffer`/`Ctp*Receipt`/`CtpTransfer`, `transferNonce`, `ctpConservationCheck`, `ctpAdvance` state machine, wire validators, `burnReceipt{To,From}Tuple`. Pure; `test/ctp.test.mjs` |
| `packages/browser/src/ctp-escrow.js` | The per-shard escrow contract + deploy programs (Phase 2) — `CTP_ESCROW_RHO` + `installProgram`/`registerProgram`/`lockProgram`/`mintProgram`/`refundProgram`/`lockOfProgram`. Modeled on `locker.js`. `--selftest` (shape); `scripts/localnet/ctp-escrow-check.mjs` (live rnode) |
