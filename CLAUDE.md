...
| `/ctp <sub>` | **Cross-shard capability transport** (#173, [CapabilityTransport.md](CapabilityTransport.md)) — a bridge is a dual-shard account, its room derived from `owner id + shard pair`. `new <shardA-url> <shardB-url> [--group <id>]` · `list` · `show` · `join <pair>` · `setup <pair>` (the escrow deploy recipe) · `escrow <pair> <A\|B> <uri>` · `send <amount> to <A\|B> [<destAddr>]` · `lock <id>` · `mint <id>` · `status` · `receipts` · `abort <id>`. Phase 3b: the room coordination + audit + `ctp-*` broadcast; the on-shard `lock`/`mint`/`refund` deploys are run by the operator via `/rholang` (recipe printed). rholang escrow is `ctp-escrow.js`, verified on a live rnode by `scripts/localnet/ctp-escrow-check.mjs` |
