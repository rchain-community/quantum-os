...
WebRTC **full mesh** over an untrusted signaling relay, with a default TURN relay
for cross-NAT calls. **Full detail — signaling trust model, room capacity
(`ROOM_HOLDS`), the single-dialer rule, leased peer IDs, `/ice`, the default TURN
relay, and every signaling-reconnect / false leave-join fix — in
[docs/connection.md](docs/connection.md).**

Load-bearing facts for code work:

- **The signaling server is an untrusted relay.** Routes SDP/ICE only (data channels are DTLS-encrypted), binds each socket to its peerId (`wsIndex`, validates `msg.from`), token-bucket rate-limited (`SIGNAL_RATE_LIMIT` — 200/s sustained, 800 burst, both the code default and the public deployment), 64 KB payload cap.
- **Full mesh, single dialer.** Every peer opens a data channel to every other. For any pair the lexicographically-**smaller** `peerId` dials (creates the pc, the data channel, the offer); the larger **only answers**, stepping in as a fallback dialler after `FALLBACK_EXTRA_MS` (15s) if nothing connects. Only one side ever offers → **no establishment glare**. `initiates(peerId) = this.peerId < peerId`, mirrored in `peer.ts` and `qospeer.mjs`. `sweep()` every 8s retries any roster peer with no open channel (skipping in-flight attempts via `connecting()`). Perfect-negotiation glare handling (`makingOffer` + polite=smaller-id) is **kept but only reachable via media renegotiation**. There is **no overlay, no flood relay, no routing** — the ring+skip overlay and `ringSkipNeighbors`/`pins`/`handleRelay`/presence-flood are all gone (they worked around a signaling rate limit that no longer bites, and their roster-divergence was the "randomly connected" failure).
- **`hasChannel(peerId)`** is the one reachability question — an open data channel or nothing. Drives the `⚠` roster mark, the status-line count, `reportUnreachable`, and `/conn`. `redial(peerId)` is the public "connect now" for the diagnostic (dials regardless of which side is the initiator).
...
- **An ICE `"failed"` OR a data-channel `onclose` to a peer still in the signaling roster is *unreachable*, not *gone*** (a phone asleep tears the channel down but stays in the room) — `cleanup()` the pc so the sweep redials, mark `⚠`, do **not** `onPeerLeft`. The authoritative "they left" is the signaling `left` message (a clean tab-close fires it within ~1s). `reestablish(peerId)` — run for every peer on `peers`/`joined` — rebuilds any connection that isn't `connected` + channel-open, so a phone waking (whose stale channel may still read `"open"`) redials rather than staying silent.
...
- **Reliability**: `qospeer.mjs` has a 30s ping/pong **keepalive** (and a 30s `"disconnected"` teardown grace — see the pegged-CPU fix in [docs/connection.md](docs/connection.md)) (reconnects a dead/zombie signaling socket — without it a long-running agent goes deaf to new joiners). Persistent identity + greet-state under `--state ./.qos-<role>`. **`run-agents.sh` / `stop-agents.sh`** launch/stop a detached set — **`facilitator` alone by default** (it carries the superset of role duties and the room's memory via `--persist`); pass roles explicitly to add more, e.g. `run-agents.sh "$ROOM" facilitator skeptic` when you want `verify`. A full mesh sustains ~15 peers and every agent spends one of those slots, so the default is one. **`room-memory.mjs`** holds the durable half of a room (lemmas, currencies, terms-series, gov groups, dyncap chains, transcript; re-served to every joiner) and has two carriers: `qos-daemon.mjs` as a peer that does nothing else, or `agent.mjs --persist <dir>` folding it into a role agent. Serving is deliberately **not** lead-gated: greeting is lead-only so N agents don't all greet, but state must be served by whoever holds it or a joiner gets nothing. `stop-agents.sh` with no arguments stops everything holding a pidfile rather than a hardcoded role list. The signaling server's per-connection rate limit is 200/s sustained / 800 burst (both the code default and the deployment) — comfortably more than a 15-peer join costs. A browser marks a peer it has no data channel to (`QOSPeer.hasChannel` → `⚠` in the roster, "N unreachable" in the status line), so the failure is visible rather than reading as "chat is broken". **Self-healing identity announce:** the agent signs its `{name, agent:<role>}` envelope **once and caches it** (`announceName`/`signedNameEnv`), so the channel-open announce and the throttled re-announce from the 30s tick loop (`reannounceStale`, ≤ once/90s per present peer) are byte-identical idempotent re-deliveries (same anchor+seq+witness — no seq advance, no fork). This recovers the single channel-open announce when it is lost to a signaling flap, which otherwise leaves a fresh browser showing the agent as an unlabelled hex id.
...
| `docs/connection.md` | Networking detail split out of this file — signaling trust, room capacity, full-mesh single-dialer rule, leased peer IDs, `/ice`, TURN, signaling-reconnect fixes |
...
| `packages/browser/test/peer.test.mjs` | **The dial state machine.** Stubbed clock, socket and `RTCPeerConnection` — no browser. Covers: the leased-id / anchor-scoping cases; the smaller-id side dials and the larger-id side only answers; an inbound offer is answered; an in-flight attempt is left alone; a stuck one is retried; a failed one is redialled; the larger-id side steps in as a fallback dialler after the grace period; a peer whose connection *fails* while still in the roster is kept and retried (not `onPeerLeft`); once the server drops them a failure does declare them gone; a departed peer is not chased; `redial()` forces a dial; `broadcast`/`send` carry no routing tags |
