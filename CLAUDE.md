...
| `/captp <sub>` | **Cross-shard value transport** (#173, [CapabilityTransport.md](CapabilityTransport.md)) — a bridge is one account (this browser's `/rholang` key) with a `captpEscrow` on two rnodes. `setup <rnode-A-url> <rnode-B-url>` (register the pair + deploy + cross-register both escrows, one step) · `list` · `show` · `new <urlA> <urlB>` (register only) · `escrow <pair> <A\|B> <rho:id:…>` · `send [<pair>] <amount> to <A\|B> [<destAddr>] [--manual]` · `lock <id> [<tuple>]` · `mint <id>` · `status` · `receipts` · `abort <id>`. `send` deploys the burn on one node and the mint on the other via `deployToNode` (`rholang.ts`, per-node result nonce) and broadcasts `captp-receipt` to the current room; `--manual` prints the `/rholang` recipe. Escrow `captp-escrow.js`, verified end to end across two real nodes with funded deploys by `scripts/localnet/captp-e2e.mjs`. `deriveBridgeRoom` + `captp-offer`/`captp-lock`/`captp-mint` are in the code, unused — Phase 4 (group-owned bridges) |
...
| `$name(args)` | Run a **chain macro** — `$verify(@x)` (local), `$balance(me)` (unsigned `/rholang eval`), `$transfer(10, "1111…")` / `$anchor(…)` (signed deploy), `$( … )` (inline program). Built-ins in `rholang-macros.js`; unknown names fall through to this room's `/macro` fragments. `$` is illegal in rholang so a missed expansion fails loud |
...
| `packages/browser/src/captp.ts` | Cross-shard transport (#173) — bridge identity + `deriveBridgeRoom` (Phase 1) and the transport protocol (Phase 3a): `CaptpOffer`/`Captp*Receipt`/`CaptpTransfer`, `transferNonce`, `captpConservationCheck`, `captpAdvance` state machine, wire validators, `burnReceipt{To,From}Tuple`. Pure; `test/captp.test.mjs` |
| `packages/browser/src/captp-escrow.js` | The per-shard escrow contract + deploy programs (Phase 2) — `CAPTP_ESCROW_RHO` + `installProgram`/`registerProgram`/`lockProgram`/`mintProgram`/`refundProgram`/`lockOfProgram`. Modeled on `locker.js`. `--selftest` (shape); `scripts/localnet/captp-escrow-check.mjs` (live rnode) |
