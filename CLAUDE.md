...
- **`$macro(…) as <pattern> { …block… }`** — capture (`expandProgram` in `rholang-macros.js`). A `capture: true` macro (`$balance` / `$transfer` / `$grant`) binds its result by `<pattern>` and runs the block instead of `return!`ing: `$balance($me) as bal { … }` → `… *ret) | for (@bal <- ret) { <block> }`. `<pattern>` is a name **or** a balanced `(a, b)` / `[a, …rest]` — whatever `for (@<pattern> <- ret)` accepts — so a `(result, error)` reply destructures directly (`$c(…) as (result, error) { … }`); the `for` fires only when the reply matches the shape. `as` gives the **raw** channel reply (`$transfer` → `Nil`/error string), so it's the composition primitive. Without `as`, the macro's own reporting runs. Non-`capture` macros reject `as`. `expand(args, sink)` — the macro splices `${sink}` where its `for` was, defaulting to its own reporting.
