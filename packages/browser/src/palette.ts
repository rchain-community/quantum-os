...
export const CMD_HELP: Record<string, string[]> = {
  ...
  captp: ["/captp — cross-shard value transport (#173, CapTP). A bridge = your /rholang key + an escrow on two rnodes.",
         "/captp setup <rnode-A-url> <rnode-B-url> — register the pair and deploy + cross-register both escrows (one step; needs /rholang key)",
         "/captp list · /captp show [<pair>] · /captp new <urlA> <urlB> (register only, no deploy)",
         "/captp send [<pair>] <amount> to <A|B> [<destAddr>] — burn on one node, mint on the other; receipt broadcasts to this room",
         "/captp status [<id>] · receipts · abort <id>. --manual prints the /rholang recipe; /captp lock/mint record a hand-run leg."],
  ...
  macro: [
    ...
    "A body of rholang makes a fragment — call it as $name(…) inside /rholang eval or deploy, where its arguments stay rholang terms. A body of plain text makes a text macro — $name just yields its body.",
    "A bare $name line for one of your macros SHOWS what it expands to (it never runs on its own); /rholang eval $name(…) or /rholang deploy $name(…) run it.",
    "/macro edit <name>[(args)] opens the rholang editor to write or rewrite the body.",
...
export const SLASH_COMMANDS: SlashCmd[] = [
  ...
  { name: "captp",   template: "/captp ",     desc: "cross-shard value transport: setup · send · status" },
