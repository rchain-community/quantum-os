...
export const CMD_HELP: Record<string, string[]> = {
  ...
  ctp: ["/ctp — cross-shard value transport (#173). A bridge = your /rholang key with an escrow on two rnodes.",
        "/ctp new <rnode-A-url> <rnode-B-url> — register a bridge (reads each node shard id) · /ctp list · /ctp show [<pair>]",
        "/ctp setup [<pair>] — deploy the escrow on both nodes and cross-register (needs /rholang key)",
        "/ctp send [<pair>] <amount> to <A|B> [<destAddr>] — burn on one node, mint on the other; receipt broadcasts to this room",
        "/ctp status [<id>] · receipts · abort <id>. --manual prints the /rholang recipe; /ctp lock/mint record a hand-run leg."],
