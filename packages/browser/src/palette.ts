...
export const CMD_HELP: Record<string, string[]> = {
  ...
  ctp: ["/ctp — cross-shard capability transport (#173). A bridge is a dual-shard account; its room is derived from your id + the shard pair.",
        "/ctp new <shardA-url> <shardB-url> [--group <id>] — create a bridge · /ctp list · /ctp show [<pair>] · /ctp join <pair>",
        "/ctp setup <pair> — the escrow deploy recipe · /ctp escrow <pair> <A|B> <rho:id:…> — record an escrow uri",
        "/ctp send <amount> to <A|B> [<destAddr>] — start a transfer (in the bridge room); then /ctp lock <id>, /ctp mint <id>",
        "/ctp status [<id>] · /ctp receipts · /ctp abort <id>. The on-shard deploys are run via /rholang (the recipe is printed)."],
...
export const SLASH_COMMANDS: SlashCmd[] = [
  ...
  { name: "ctp",     template: "/ctp ",       desc: "cross-shard transport: new · setup · send · status" },
