...
- On the signaling `peers` / `joined` message: `reestablish(peerId)` for each — if the connection isn't fully healthy (`connectionState === "connected"` **and** the channel open) it drops whatever stale pc exists and re-dials per the rule. A `joined` for a peer we already hold a connection to means **they reconnected** (a phone waking from sleep, a reload): their ICE session is fresh but a data channel can still read `"open"` while pointing at a frozen tab, so without this nothing redials and messages to them vanish. `onPeerJoined` still fires for every peer unfiltered, so the app's roster shows everyone present.
- **A data-channel `onclose` for a peer still in the signaling roster is *not* a departure** — a phone that sleeps tears its SCTP association down but is still in the room. Keep them (⚠, retried), same as an ICE `"failed"` while still rostered; only the signaling `left` message (which a clean tab-close triggers within ~1s via the socket closing) declares them gone.
...
- **Presence eviction (stale-room fix)**: an *ungraceful* drop sends no `leave`. `"disconnected"` starts an 8s grace timer and evicts only if it doesn't recover. `declarePeerGone` is idempotent.
- **An ICE `"failed"` or a data-channel `onclose` to a peer still in the roster is *unreachable*, not *gone*.** `"failed"` means *no candidate pair worked*; a channel `onclose` is what a phone going to sleep produces (the SCTP association tears down) — neither is "they left". If `roster.has(peerId)` → `cleanup()` the dead pc so the sweep redials, back `retryAt` off one cycle, and **return without `onPeerLeft`**; `renderPeers` marks them `⚠`. The authoritative "they left" is the signaling `left` message, which a clean tab-close fires within ~1s (the WebSocket closes → the server broadcasts `left`). `test/peer.test.mjs` covers the phone-sleep-then-wake round trip. On wake, `reestablish()` (run for every peer on the `peers`/`joined` message) rebuilds the stale connection.
