...
**`$macro(…) as name { …block… }` — capture.** A macro that opts in (`capture: true` — `$balance`, `$transfer`, `$grant`) can bind its result to `name` and run a supplied block instead of reporting to `return`: `$balance($me) as bal { stdout!(("balance", bal)) }` expands to `… *ret) | for (@bal <- ret) { stdout!(("balance", bal)) }`. Without `as`, the macro reports as before (`$transfer`'s friendly "transfer ok" / a value on `return` for eval / the deploy record). With `as`, you get the **raw** channel reply (`$transfer` → `Nil` or an error string) and full control — the composition primitive: `$transfer(10, a) as r { $balance($me) as b { return!(("done", r, b)) } }`. A non-`capture` macro rejects `as`. The block's braces are balanced (nested `match … { }` is fine); an unbalanced one is a reported error.
